...
export default function DescriptionsPro(props: DescriptionsProProps) {
  ...
  // 先筛选一下
  const showSchema = schema.filter(item => {
    if (Object.prototype.hasOwnProperty.call(item, "hide")) {
      return typeof item.hide === "boolean"
        ? !item.hide
        : !(item.hide as Function)(item, schema);
    }
    return true;
  });

  const middle = matcher(showSchema)
    .addKeyFn("content", (val: Schema) => {
      if (Object.prototype.hasOwnProperty.call(val, "render")) {
        return (val.render as (val: any, record: any) => ReactNode)(
          data[val.dataIndex],
          data
        );
      }
  ...
  let sortedData = (middle.data as Record<string, any>[]).sort(
  ...
  return (
    <div
      className={`${
        center ? "text-align-center descriptions-pro" : "descriptions-pro"
      }  `}
    >
      {Object.keys(data).length > 0 && !groups ? (
        <Descriptions {...other}>
          {sortedData.map((item, index) => {
            const { content, ...otherItem } = item;
            return (
              <Item key={`${index}`} {...otherItem}>
                {item.content}
              </Item>
            );
          })}
      ...
      {Object.keys(data).length > 0 && groups ? (
        <div>
          {sortedData.map((group, index) => (
            <Descriptions {...group.props} key={`${index}`}>
